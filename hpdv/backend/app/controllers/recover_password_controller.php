...
if ($service_user->recoverPassword()) {
    ...
    if ($saveTokenResetPassword) {
        ...
        $mensagem = "<html>
                        ...
                        <body>
                            <div class=\"container\">
                                <h2>Recuperação de senha - NextPulse PDV</h2>
                                <p>Você solicitou a recuperação de senha do NextPulse PDV.
        ...
        $mail->Host       = 'smtp.gmail.com';  // SEU HOST (HOSPEDAGEM) - nesse caso deixei para o gmail
        $mail->SMTPAuth   = true;                        // Manter em true
        $mail->Username   = 'amadoxonan@gmail.com';
        $mail->Password   = '@Natural3898';
        ...
        //Recipients
        $mail->setFrom('amadoxonan@gmail.com', 'NextPulse PDV');  //DEVE SER O MESMO EMAIL DO USERNAME
        $mail->addAddress($email_recover);     // QUAL EMAIL RECEBERÁ A MENSAGEM!
        //$mail->addBCC(''); //ADICIONANDO BCC

        // Content
        $mail->isHTML(true);                                  // Set email format to HTML
        $mail->Subject = 'Recuperação de senha NextPulse PDV'; //ASSUNTO
